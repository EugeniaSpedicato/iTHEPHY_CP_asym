...
data6->plotOn(neg_sides_frame);
  ...
  RooRealVar *N = new RooRealVar("N", "N", 0.0058, 0., 5.);
  ...
  RooAbsPdf *arg_neg = RooClassFactory::makePdfInstance("arg_neg", "1/N*pow(dtf_neg-a,b)*exp(-c*(x-a))", RooArgSet(*dtf_neg, *N, *a, *b, *c));
  ...
  RooAbsPdf *arg_pos = RooClassFactory::makePdfInstance("arg_pos", "1/N*pow(dtf_pos-a,b)*exp(-c*(x-a))", RooArgSet(*dtf_pos, *N, *a, *b, *c));
